...
const MediumClap = ({
  ...
  const componentJustMounted = useRef(true); // 처음 마운트 됐을때 useEffect안의 내용이 동작하지 않도록 하게

  useEffect(() => {
    if (!componentJustMounted.curren) {
...
const ClapIcon = ({ style: userStyles = {}, className }) => {
  ...
  const classNames = [styles.icon, isClicked ? styles.checked : "", className]
    .join(" ")
    .trim();
  console.log("icon classses", classNames);
  return (
    <span>
      <svg
        id="clapIcon"
...
const Usage = () => {
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      {!!count && (
        <div className={styles.info}>{`You hava Clapped ${count} times`}</div>
      )}
    </div>
  );
};
